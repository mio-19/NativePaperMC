...
public class ${plug_name} {
  public static URL classGetResource(Class clazz, String path) {
    if (path.startsWith("/")) {
      URL url = clazz.getResource("/${dupres_path}" + path);
      if (url != null) {
        return url;
      }
    }
  ...
  public static InputStream classGetResourceAsStream(Class clazz, String path) {
    if (path.startsWith("/")) {
      InputStream stream = clazz.getResourceAsStream("/${dupres_path}" + path);
      if (stream != null) {
        return stream;
      }
    }
...
async function minify_plugins() {
  for (const jar of (await ls("dist/plugins/*.jar")).concat(["dist/mc.jar"])) {
